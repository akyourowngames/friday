...
import { ArrowDown, ArrowUp, FileText, Paperclip, Square, X } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import type { ActivityEvent, ChatMessage, PendingFile, TraceEvent } from "@/lib/types";
...
function markdown(value: string) {
  ...
  return escaped
    ...
    .replace(/\*([^*]+)\*/g, "<em>$1</em>")
    .split(/\n{2,}/).map(block => block.startsWith("<pre") || block.startsWith("<h") ? block : `<p>${block.replace(/\n/g, "<br>")}</p>`).join("");
}
...
export function ChatView(props: Props) {
  const { messages, historyLoading, streaming, busy, activity, input, setInput, attachments, removeAttachment, addFiles, sendMessage, model, traces } = props;
  ...
  const activityTitle = streaming ? "Ares is writing" : busy ? "Ares is thinking" : "Run activity";
  ...
  return <section className="view is-active chat-view">
      <div className="chat-main" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); addFiles(event.dataTransfer.files); }}>
        <div className="messages" ref={scrollRef} aria-live="polite" onScroll={event => { const element = event.currentTarget; setAtBottom(element.scrollHeight - element.scrollTop - element.clientHeight < 80); }}>
          {historyLoading ? <div className="history-skeleton" aria-label="Loading conversation" role="status">
            ...
            <div>
              ...
              <div className="message-copy" dangerouslySetInnerHTML={{ __html: markdown(message.content) }} />
          ...
          {activity.length && !historyLoading ? <section className={`run-log ${busy ? "is-live" : "is-complete"}`} aria-label="Ares run activity">
            <header><span className="thinking-orb"><i /><i /><i /></span><strong>{activityTitle}</strong><small>{busy ? "live" : "saved"}</small></header>
            <div>{activity.slice(-6).map(item => <div className={`activity-item is-${item.state}`} key={item.id}><span /><p><strong>{item.label}</strong>{item.detail && <small>{item.detail}</small>}</p>{item.kind === "tool" && <em>tool</em>}</div>)}</div>
          </section> : null}
          {busy && !streaming && !historyLoading ? <article className="message assistant thinking-message"><div className="message-avatar">A</div><div><div className="message-head"><strong>Ares</strong><time>reasoning</time></div><div className="thinking-copy"><span>Thinking through this</span><i /><i /><i /></div></div></article> : null}
          {streaming ? <article className="message assistant"><div className="message-avatar">A</div><div><div className="message-head"><strong>Ares</strong><time>thinking live</time></div><div className="message-copy" dangerouslySetInnerHTML={{ __html: markdown(streaming) + '<span class="stream-caret"></span>' }} /></div></article> : null}
        </div>
        ...
        <div className="composer-dock">
          {activeTrace && <div className="inline-trace"><span className="connection-dot is-online" /><strong>{activeTrace.label}</strong><span>{activeTrace.detail || "Working…"}</span></div>}
          ...
          <div className="composer">
            ...
            <div className="composer-actions"><button className="mode-select" title={model}><span className="model-dot" /><span>{model || "Model"}</span></button><button className={`send-btn ${busy ? "is-stop" : ""}`} onClick={() => sendMessage()} aria-label={busy ? "Ares is responding" : "Send message"}>{busy ? <Square size={13} fill="currentColor" /> : <ArrowUp />}</button></div>
          </div>
          <div className="composer-meta"><span>Ares can make mistakes. Check important information.</span><span>{attachments.length ? `${attachments.length} file${attachments.length === 1 ? "" : "s"} ready` : "Files stay local"}</span></div>
