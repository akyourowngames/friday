...
class SarvamTranscriber:
    ...
    def transcribe_file(self, path: Path) -> TranscriptResult:
        ...
        if response.status_code >= 400:
            raise RuntimeError(f"Sarvam STT HTTP {response.status_code}: {response.text[:400]}")
...
class AudioRecorder:
    ...
    def stop(self) -> Path | None:
        ...
        if not started_at or time.perf_counter() - started_at < self.settings.stt_min_seconds:
            return None
        with self._lock:
    ...
    def _write_wav(self, samples: np.ndarray) -> Path:
        out_dir = Path(self.settings.stt_output_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        path = out_dir / f"friday-input-{int(time.time())}-{uuid4().hex[:8]}.wav"
        path.write_bytes(samples_to_wav(samples, self.settings.stt_sample_rate))
        return path


class SpaceHoldToTalk:
    def __init__(self, settings: Settings, on_transcript, on_status) -> None:
        self.settings = settings
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.on_status("Voice input disabled: install keyboard.")
            return False
