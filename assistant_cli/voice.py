...
class SarvamVoice:
    def __init__(self, settings: Settings, enabled: bool = False) -> None:
    ...
    def status(self) -> dict[str, str]:
        return {
            "mode": "voice" if self.enabled else "text",
    ...
    def synthesize(self, text: str) -> VoiceResult:
        ...
        if response.status_code >= 400:
            raise RuntimeError(f"Sarvam TTS HTTP {response.status_code}: {response.text[:400]}")

        data = response.json()
        audios = data.get("audios") or []
        if not audios:
            raise RuntimeError("Sarvam TTS response had no audio.")

        audio = base64.b64decode(str(audios[0]))
        out_dir = Path(self.settings.voice_output_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        suffix = "." + self.settings.voice_codec.strip().lower().lstrip(".")
        path = out_dir / (datetime.now().strftime("friday-%Y%m%d-%H%M%S-") + uuid4().hex[:8] + suffix)
    ...
    def play(self, path: Path, wait: bool = True) -> None:
        ...
        if os.name == "nt":
            ...
            return
        raise RuntimeError(f"No local audio player configured for {path}.")
    ...
    def _speech_text(self, text: str) -> str:
        ...
        if len(speech) <= max_chars:
            return speech
        return speech[:max_chars].rsplit(" ", 1)[0].strip()
