...
function shouldAutoOpenActivityPanel() {
    if (!settings.autoOpenActivity) return false;
    if (typeof window === 'undefined' || !window.matchMedia) return true;
    return window.matchMedia('(min-width: 769px)').matches;
}

const TOOL_VISUAL_LABELS = {
    web_search: 'WEB',
    web_fetch: 'FETCH',
    reddit: 'REDDIT',
    hackernews: 'HN',
...
function ensureToolStage() {
    let stage = document.getElementById('tool-stage');
    if (!stage) {
    ...
    let hud = document.getElementById('tool-hud');
    if (!hud) {
        hud = document.createElement('div');
        hud.id = 'tool-hud';
        hud.className = 'tool-hud';
        hud.innerHTML = '<span class="tool-hud__dot"></span><strong>CORE</strong><em>Standing by</em>';
        document.body.appendChild(hud);
    }
    return { stage, hud };
}

function toolKindFromActivity(activity) {
    const tool = String(activity.tool_name || activity.tool || '').toLowerCase();
    ...
    if (combined.includes('web_fetch')) return 'web_fetch';
    if (combined.includes('browser')) return 'browser';
    if (combined.includes('terminal') || combined.includes('system')) return 'terminal';
    if (combined.includes('file')) return 'file_read';
    if (combined.includes('vision') || combined.includes('camera')) return 'vision';
    if (route === 'realtime') return 'web_search';
    if (route === 'task') return 'terminal';
    return 'assistant';
}

function shouldAnimateToolEvent(activity) {
    const eventName = activity && activity.event;
    return eventName === 'tool_chain_planned'
        ...
        || eventName === 'context_retrieved'
        || eventName === 'first_chunk';
}

function triggerToolAnimation(activity) {
    if (!activity || !shouldAnimateToolEvent(activity)) return;
    const { stage, hud } = ensureToolStage();
    const kind = toolKindFromActivity(activity);
    const label = TOOL_VISUAL_LABELS[kind] || TOOL_VISUAL_LABELS.assistant;
    const detail = activity.message || 'Tool layer active';
    const burst = document.createElement('div');
    burst.className = `tool-burst tool-burst--${kind}`;
    burst.innerHTML = `
        <span class="tool-burst__beam"></span>
        <span class="tool-burst__ring tool-burst__ring--one"></span>
        <span class="tool-burst__ring tool-burst__ring--two"></span>
        <span class="tool-burst__node tool-burst__node--one"></span>
        <span class="tool-burst__node tool-burst__node--two"></span>
        <span class="tool-burst__node tool-burst__node--three"></span>
        <strong>${escapeHtml(label)}</strong>
        <em>${escapeHtml(detail)}</em>`;
    stage.appendChild(burst);
    hud.className = `tool-hud tool-hud--${kind} is-visible`;
    hud.innerHTML = `<span class="tool-hud__dot"></span><strong>${escapeHtml(label)}</strong><em>${escapeHtml(detail)}</em>`;
    setTimeout(() => burst.remove(), 1500);
    clearTimeout(triggerToolAnimation._hideTimer);
    triggerToolAnimation._hideTimer = setTimeout(() => {
...
function renderSearchResults(payload) {
    ...
    triggerToolAnimation({
        ...
        route: 'realtime',
        tool_name: payload.source || 'evidence',
        message: payload.answer || 'Evidence panel ready.',
    });
    if (searchResultsWidget) {
        searchResultsWidget.classList.remove('tool-panel-flash');
        void searchResultsWidget.offsetWidth;
        searchResultsWidget.classList.add('tool-panel-flash');
        setTimeout(() => searchResultsWidget.classList.remove('tool-panel-flash'), 1100);
    }
...
function appendActivity(activity) {
    ...
    if (activity.event === 'query_detected') {
        ...
        addRouteClass(activity.route);
    } else if (activity.event === 'waiting_for_model') {
        const ms = activity.elapsed_ms;
        const elapsed = ms != null ? ` (${ms < 1000 ? ms + ' ms' : (ms / 1000).toFixed(1) + ' s'})` : '';
        detail = (activity.message || 'Waiting for model response...') + elapsed;
        item.classList.add('activity-sub');
