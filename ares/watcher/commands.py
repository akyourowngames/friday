...
class WatcherCommands:
    ...
    def execute(self, argument: str) -> dict[str, Any]:
        ...
        if action == "add":
            return {"action":"add","monitor":self._add(tokens).public_dict()}
        if not tokens:
            raise ValueError(f"{action} requires a monitor ID")
        monitor = self._resolve(tokens[0])
        if action in {"status", "show"}:
            return {"action":"status","monitor":monitor.public_dict(),"events":[item.to_dict() for item in self.db.list_events(monitor.id,limit=5)]}
        ...
        if action in {"remove", "delete", "rm"}:
            ...
            return {"action":"remove","monitor":monitor.public_dict()}
        if action == "events":
    ...
    def _add(self, tokens: list[str]) -> Monitor:
        positional, options = [], {}
        index = 0
        ...
        if len(positional) < 2:
            raise ValueError('Usage: /monitor add "NAME" URL [--interval 15m] [--type website] [--action notify]')
