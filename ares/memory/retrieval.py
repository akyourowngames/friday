...
class MemoryQueryRewriter:
    ...
    async def rewrite(self, user_message: str, recent_history: list[dict]) -> str:
        ...
        try:
            ...
            from ares.integrations.llm import LLMClient
            rewritten = self._parse(str(LLMClient.extract_content(response)))
            reference_text = " ".join(
...
class ActiveMemoryJudge:
    ...
    async def judge(
        ...
        try:
            ...
            from ares.integrations.llm import LLMClient
            ids = self._parse_ids(str(LLMClient.extract_content(response)))
...
class MemoryRecallService:
    ...
    async def prepare(
        ...
        *,
        ...
        try:
            # The ONNX/Sentence-Transformers embedding forward pass is the real
            # latency hotspot on the response critical path: it blocks first-token
            # delivery for the whole turn when run on the asyncio event loop.  The
            # SQLite queries inside ``search`` are sub-millisecond, so only the
            # embedding needs to leave the loop.  We precompute the query vector in
            # a worker thread and pass it straight back into ``search`` — which
            # keeps all SQLite work on the loop, avoiding the cross-thread
            # ``check_same_thread`` hazard on the connection.  ``to_thread`` is a
            # no-op cost when we are already off-loop.
            use_semantic = self._embedding_ready if not foreground_models else None
            query_vector: bytes | None = None
            if use_semantic and self.memory_store.vector_enabled:
                query_vector = await asyncio.to_thread(
                    self.memory_store._embed_query, rewritten
                )
