...
from ares.skills.actions import extract_since_reference, has_reference_language
from ares.context.blend import build_context_prompt, get_model_budgets
...
def build_user_context(
    ...
    *,
    ...
    if memories_override is _MEMORIES_UNSET:
        ...
        # Always include high-importance standing memories so Ares remembers
        # the user's key facts even when the query doesn't match them.
        try:
            standing = memory_store.get_standing_memories(
                limit=int(getattr(config, "standing_memory_limit", 8)),
                min_importance=float(getattr(config, "standing_memory_min_importance", 0.3)),
            )
            seen_ids = {int(m.get("fact_id", -1)) for m in memories}
            for mem in standing:
                if int(mem.get("fact_id", -1)) not in seen_ids:
                    memories.append(mem)
        except Exception:
            pass
    else:
