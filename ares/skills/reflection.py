...
from ares.integrations.llm import LLMClient, normalize_provider, provider_for_model
from ares.memory.lifecycle import (
    ...
    async def extract(
        ...
        *,
        ...
        if isinstance(self.llm, LLMClient):
            ...
            if review_model:
                # Cross-provider routing: the LLM client resolves the correct
                # endpoint (base URL + API key) per model at call time.
                chat_kwargs.update({
                    "model": review_model,
        ...
        # Some reasoning models put output in reasoning_content, not content.
        content_text = (
            response.get("content")
            or response.get("reasoning_content")
            or "{}"
        )
        return self._parse_json(str(content_text))
