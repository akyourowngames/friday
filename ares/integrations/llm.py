...
# ``load_config`` is imported lazily inside the function that uses it.  Importing
# it at module load would create a circular import: ares.config imports
# ares.integrations.browser at top level, which re-enters ares.config while it is
...
class LLMClient:
    ...
    def extract_content(response: dict, default: str = "") -> str:
        ...
        Handles both standard ``content`` and reasoning-model
        ``reasoning_content`` fields.  Some free models (e.g.
        ``deepseek-v4-flash-free``) are reasoning models that put their
    ...
    def _copilot(self):
        from ares.integrations.copilot import CopilotLLMClient
    ...
    def _resolve_endpoint(self, model: str | None) -> tuple[str, str]:
        """Return (base_url, api_key) for *model*, handling cross-provider routing.

        When the fast conversation model belongs to a different provider than
        the main client (e.g. an OpenCode free model while the main provider
        is NIM), we route to the correct endpoint automatically.
        """
        if not model:
            return self.base_url, self.api_key
        target_provider = provider_for_model(model)
        active_provider = normalize_provider(self.provider)
        if target_provider and normalize_provider(target_provider) != active_provider:
            base_url = resolve_provider_base_url(target_provider)
            api_key = configured_provider_api_key(self.config, target_provider)
    ...
    async def chat(
        ...
        *,
        ...
        # Cross-provider routing: resolve endpoint for the target model
        endpoint_url, endpoint_key = self._resolve_endpoint(target_model)
        ...
        for attempt in range(3):
            # Re-resolve endpoint each attempt in case the model changed via fallback
            endpoint_url, endpoint_key = self._resolve_endpoint(payload["model"])
    ...
    async def chat_stream(
        ...
        *,
        ...
        emitted = False
        request_timeout = httpx.Timeout(timeout if timeout is not None else 60.0)
        for attempt in range(3):
            # Re-resolve endpoint each attempt in case the model changed via fallback
            endpoint_url, endpoint_key = self._resolve_endpoint(payload["model"])
            headers["Authorization"] = f"Bearer {endpoint_key}" if endpoint_key else ""
